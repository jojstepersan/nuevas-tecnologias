 ...
 * @author Stiven
 */
@WebServlet(name = "AddImageExpositor", urlPatterns = {"/AddImageExpositor"})
    public class AddImageExpositor extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        for(int i=0;i<items.size();i++)
            ...
            if(!item.isFormField())
                {
                regis_exp.expositor.setImage(item.getName()); 
        ...
        conexion.setExpositor(regis_exp.expositor);
        processRequest(request, response);
